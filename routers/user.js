  ...
  return new Promise(
    (resolve, reject) => {
      ...
      .then((db) => {
        ...
        db.usuarios = require('../models/usuarios.js')(db.sequelize,db.Sequelize); 
        db.productos = require('../models/productos.js')(db.sequelize,db.Sequelize); 
        db.usuarios.hasMany(db.productos, {foreignKey: 'usuario_id'})

        resolve(entity.getParentAndChilds(db.usuarios, db.productos));
      })
      .catch((ex) => {
...
//         resolve(entity.getEntity(db.usuarios,req.pathParams ));
//       })
//       .catch((ex) => {
//         console.log(`Error: ${ex.message}`);
//         reject( `Ok, something went seriously wrong...\n ${ex.message}`);
...
api.post('/usuarios',  (req) =>{
  ...
  return new Promise(
    (resolve, reject) => {
      ...
      .then((db) => {
        console.log(`On load database`);
        resolve(entity.addEntity(db.usuarios, req.body));
        //resolve(db);
      })
      .catch((ex) => {
        console.log(`Error: ${ex.message}`);
...
api.patch('usuarios', (req) => {
  return new Promise(
    (resolve, reject) => {
      dbInstance
      .then((db) => {
        console.log(`On load database`);
        resolve(entity.updateEntity(db.usuarios, req.body));
      })
      .catch((ex) => {
        console.log(`Error: ${ex.message}`);
        reject( `Ok, something went seriously wrong...\n ${ex.message}`);
...
api.delete('usuarios', (req) => {
  return new Promise(
    (resolve, reject) => {
      dbInstance
      .then((db) => {
        console.log(`post load database`);
        resolve(entity.deleteEntity(db.usuarios, req.body));
      })
      .catch((ex) => {
        console.log(`Error: ${ex.message}`);
        reject( `Ok, something went seriously wrong...\n ${ex.message}`);
